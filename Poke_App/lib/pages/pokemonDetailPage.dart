...
//   }
  ...
  void getCurrentUser() async{
    ...
    await databaseReference
      ...
      if(currentUser.favorites.contains(model.orderId)){
        isFavourite = true;
      }else{
        isFavourite = false;
      }
  ...
  void saveFavoritePokemon() async {
    if(currentUser.favorites.contains(model.orderId)){
      isFavourite = false;
      currentUser.favorites.remove(model.orderId);
    }else{
      isFavourite = true;
      currentUser.favorites.add(model.orderId); 
    }
    
    await databaseReference
      .collection('usuarios')
      .document(currentUser.uid).updateData({'favorites': currentUser.favorites});
      setState(() {});
  }
}
